[![Build Status](https://github.com/iungopbx/iungo-web-phone/actions/workflows/ci.yml/badge.svg?branch=master)](https://github.com/iungopbx/iungo-web-phone/actions/workflows/ci.yml)

# Iungo WebPhone Library

The Iungo WebPhone Library includes a JavaScript WebRTC library and a WebRTC phone demo app.

## Prerequisites

- You will need an active Iungo account. Don't have an account? [Get your Free Iungo Developer Account Now!](https://developers.iungo.com)
...
2. [Usage](#usage)
  1. [Configuring your Iungo app](#configuring-your-iungo-app)
...
2. Download WebPhone SDK: [https://github.com/iungopbx/iungo-web-phone/releases/latest](https://github.com/iungopbx/iungo-web-phone/releases/latest)
3. Download audio files:
    1. [https://cdn.rawgit.com/iungopbx/iungo-web-phone/master/audio/incoming.ogg](https://cdn.rawgit.com/iungopbx/iungo-web-phone/master/audio/incoming.ogg)
    2. [https://cdn.rawgit.com/iungopbx/iungo-web-phone/master/audio/outgoing.ogg](https://cdn.rawgit.com/iungopbx/iungo-web-phone/master/audio/outgoing.ogg)

---

## Usage

### Configuring your Iungo app
...
Since WebRTC enables dialing out, you need to have a `DIGITAL LINE` attached to an extension to use this capability. You can configure this in Online Web Portal for [Production](https://service.iungo.com/) and [Sandbox](https://service.devtest.iungo.com/) accounts. More information on Digital Lines and their configuration is available in the following Iungo Knowledge Base article topics:

1. [Digital Line Overview (KB 5862)](http://success.iungo.com/articles/en_US/RC_Knowledge_Article/5862)
2. [Adding a Digital Line (KB 3136)](http://success.iungo.com/articles/RC_Knowledge_Article/5-10-Adding-Phones-to-other-extensions-via-Web). A limited number of Digital Lines are free with each sandbox account which can be configured with the free Iungo for Desktop softphone.
3. [Reassigning an Existing Digital Line (KB 3748)](http://success.iungo.com/articles/en_US/RC_Knowledge_Article/How-to-Assign-an-Existing-Digital-Line-to-a-different-extension)

These permissions be configured for your app in the [Iungo Developer Portal](https://developers.iungo.com/). Fill this [Registration Form](https://docs.google.com/forms/d/15kK_zJ5FhyXiH8gwOqiaG7_BuTWGCeeVr4MAv4OBpUM/viewform) to get access to WebRTC permissions. Please contact devsupport@iungo.com to request these permissions.
...
<script src=".../iungo-web-phone.js" type="text/javascript"></script>
```

### Application

For this example you will also need to have [Iungo JS SDK installed](https://github.com/iungopbx/iungo-js#installation).
...
var sdk = new Iungo.SDK({
    ...
    server: Iungo.SDK.server.production // or .sandbox
});
    ...
    .then(function(loginResponse) {
        ...
        return platform
            ...
            .then(function(res) { // Doing nested then because we need loginResponse in a simple way
                ...
                return new Iungo.WebPhone(res.json(), { // optional
                    clientId: clientId,
                    appName: appName,
                    appVersion: appVersion,
...
2. If your Application is of the Scope
   `Server/Web`
   `Browser-Based`
    Then you would need to add `http://localhost:8080/callback.html` as the OAuth Redirect URI for the application in [Developer Portal](https://developer.iungo.com)
...
```js
var webPhone = new Iungo.WebPhone(provisionData, options);
```
...
Before:
```javascript
webPhone = new Iungo.WebPhone(data, {
