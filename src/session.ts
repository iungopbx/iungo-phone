...
import {IungoWebPhoneUserAgent} from './userAgent';
...
/**
 * Object representing all the headers used by Iungo backend
 */
export interface IungoHeaders {
    body?: string;
    reqid?: string;
    sid?: string;
    request?: string;
...
export class CommonSession {
    ...
    /** JOSN representation of Iungo headers received for an incoming call */
    iungoHeaders?: IungoHeaders;
    __qosStats?: QosStats;
    ...
    /** Iungo barge implementation */
    barge?: typeof barge;
    /** Iungo blind transfer implementation */
    ...
    /**
     * @internal
     * Create session message which would be sent to the Iungo backend
     */
    ...
    /** Iungo reply with message implementation */
    ...
    /**
     * @internal
     * Helper function to send INFO request with `move` instruction to Iungo backend
     */
    ...
    /** Iungo warm transfer implementation */
    warmTransfer?: typeof warmTransfer;
    /** Iungo whisper implementation */
    whisper?: typeof whisper;
    setQosStats?: typeof setQosStats;
}

export type IungoWebPhoneSession = IungoWebPhoneInvitation | IungoWebPhoneInviter;
 ...
 */
export interface IungoWebPhoneInvitation extends Invitation, CommonSession {
    /**
     ...
     * User Agent instance
     */
    userAgent: IungoWebPhoneUserAgent;
}
 ...
 */
export interface IungoWebPhoneInviter extends Inviter, CommonSession {
    /**
     * User Agent instance
     */
    userAgent: IungoWebPhoneUserAgent;
...
export function patchWebphoneSession(session: IungoWebPhoneSession): IungoWebPhoneSession {
...
export function patchIncomingWebphoneSession(session: IungoWebPhoneSession): void {
    try {
        parseRcHeader(session);
    } catch (e) {
        (session as any).logger.error("Can't parse Iungo headers from invite request due to " + e);
    }
    ...
    (session as IungoWebPhoneInvitation).accept = accept.bind(session);
    setupUserAgentCoreEvent(session);
}

function canUseRCMCallControl(this: IungoWebPhoneSession): boolean {
    return !!this.iungoHeaders;
}

function createSessionMessage(this: IungoWebPhoneSession, options: IungoHeaders): string {
    if (!this.iungoHeaders) {
        return undefined;
    }
    extend(options, {
        sid: this.iungoHeaders.sid,
        request: this.iungoHeaders.request,
        from: this.iungoHeaders.to,
        to: this.iungoHeaders.from
    });
    return this.userAgent.createRcMessage(options);
}

async function sendReceiveConfirm(this: IungoWebPhoneSession): Promise<IncomingResponse> {
...
function sendSessionMessage(this: IungoWebPhoneSession, options): Promise<IncomingResponse> {
    if (!this.iungoHeaders) {
        (this as any).logger.error("Can't send SIP MESSAGE related to session: no Iungo headers available");
    }
    return this.userAgent.sendMessage(this.iungoHeaders.from, this.createSessionMessage(options));
}

async function sendInfoAndReceiveResponse(this: IungoWebPhoneSession, command: Command, options?: any): Promise<any> {
    options = options || {};
...
async function startRecord(this: IungoWebPhoneSession): Promise<any> {
    return setRecord(this, true);
}

async function stopRecord(this: IungoWebPhoneSession): Promise<any> {
    return setRecord(this, false);
}

function sendMoveResponse(
    this: IungoWebPhoneSession,
    reqId: number,
    code: number,
    description: string,
...
function ignore(this: IungoWebPhoneSession): Promise<IncomingResponse> {
    return this.sendReceiveConfirm().then(() => this.sendSessionMessage(messages.ignore));
}

function toVoicemail(this: IungoWebPhoneSession): Promise<IncomingResponse> {
    return this.sendReceiveConfirm().then(() => this.sendSessionMessage(messages.toVoicemail));
}

function replyWithMessage(this: IungoWebPhoneSession, replyOptions: ReplyOptions): Promise<IncomingResponse> {
...
async function flip(this: IungoWebPhoneSession, target): Promise<any> {
    return this.sendInfoAndReceiveResponse(messages.flip, {target});
}

async function whisper(this: IungoWebPhoneSession): Promise<any> {
    return this.sendInfoAndReceiveResponse(messages.whisper);
}

async function barge(this: IungoWebPhoneSession): Promise<any> {
    return this.sendInfoAndReceiveResponse(messages.barge);
}

function park(this: IungoWebPhoneSession): Promise<any> {
    return this.sendInfoAndReceiveResponse(messages.park);
}

function mute(this: IungoWebPhoneSession, silent?: boolean): void {
...
function unmute(this: IungoWebPhoneSession, silent?: boolean): void {
    if (this.state !== SessionState.Established) {
...
function addTrack(this: IungoWebPhoneSession, remoteAudioEle?, localAudioEle?): void {
...
function stopMediaStats(this: IungoWebPhoneSession): void {
...
async function blindTransfer(
    this: IungoWebPhoneSession,
    target: string | URI | IungoWebPhoneSession,
...
/**
 *
 * @param this IungoWebPhoneSessionSessionInviteOptions
 * @param options
 ...
 */
function reinvite(this: IungoWebPhoneSession, options: SessionInviteOptions = {}): Promise<OutgoingInviteRequest> {
...
async function hold(this: IungoWebPhoneSession): Promise<void> {
    this.stopMediaStats();
    try {
...
async function unhold(this: IungoWebPhoneSession): Promise<void> {
    try {
...
function dtmf(this: IungoWebPhoneSession, dtmf: string, duration = 100, interToneGap = 50): void {
...
async function accept(this: IungoWebPhoneSession, options: InvitationAcceptOptions = {}): Promise<IungoWebPhoneSession> {
    options = options || {};
...
async function forward(
    this: IungoWebPhoneSession,
    target: IungoWebPhoneSession,
    acceptOptions: InvitationAcceptOptions,
    transferOptions: SessionReferOptions
): Promise<OutgoingReferRequest> {
    await (this as IungoWebPhoneInvitation).accept(acceptOptions);
...
async function dispose(this: IungoWebPhoneSession) {
...
function parseRcHeader(session: IungoWebPhoneSession): void {
    const prc = session.request.getHeader('P-Iungo');
    const prcCallInfo = session.request.getHeader('P-Iungo-Api-Call-Info');
    if (prc) {
    ...
    if (prcCallInfo) {
        ...
        extend(session.iungoHeaders, parsed);
    }
}

async function setRecord(session: IungoWebPhoneSession, flag: boolean): Promise<any> {
...
function enableReceiverTracks(session: IungoWebPhoneSession, enable: boolean): void {
...
function enableSenderTracks(session: IungoWebPhoneSession, enable: boolean): void {
    const sessionDescriptionHandler = session.sessionDescriptionHandler as SessionDescriptionHandler;
    const peerConnection = sessionDescriptionHandler.peerConnection;
    if (!peerConnection) {
        (session as any).logger.error('Peer connection closed.');
...
function setHold(session: IungoWebPhoneSession, hold: boolean): Promise<void> {
    return new Promise((resolve, reject) => {
...
function stopPlaying(session: IungoWebPhoneSession): void {
    session.userAgent.audioHelper.playOutgoing(false);
    session.userAgent.audioHelper.playIncoming(false);
}

export function onSessionDescriptionHandlerCreated(session: IungoWebPhoneSession): void {
...
function setupUserAgentCoreEvent(session: IungoWebPhoneSession) {
...
function stopMediaStreamStats(session: IungoWebPhoneSession) {
...
function onLocalHold(this: IungoWebPhoneSession): boolean {
    return this.__localHold;
}

function setQosStats(this: IungoWebPhoneSession, stats: QosStats) {
