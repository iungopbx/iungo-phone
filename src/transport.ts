...
import {TransportServer, IungoWebPhoneOptions} from './index';
import {Events} from './events';

export interface IungoWebPhoneTransport extends Transport {
    /** @ignore */
    configuration?: TransportOptions;
    /** logger class to log transport related messaged */
    logger?: Logger;
    /**
     * Address of the Iungo main proxy
...
export function createIungoWebPhoneTransport(transport: IungoWebPhoneTransport, options: IungoWebPhoneOptions): IungoWebPhoneTransport {
...
function __connect(this: IungoWebPhoneTransport): Promise<void> {
...
function __setServerIsError(this: IungoWebPhoneTransport, uri: string): void {
...
function __resetServersErrorStatus(this: IungoWebPhoneTransport): void {
    this.servers.forEach(server => {
        server.isError = false;
    });
}

function __isCurrentMainProxy(this: IungoWebPhoneTransport): boolean {
...
function __onConnectedToMain(this: IungoWebPhoneTransport): void {
    this.__clearSwitchBackToMainProxyTimer();
}

function __onConnectedToBackup(this: IungoWebPhoneTransport): void {
    if (!this.switchBackToMainProxyTimer) {
        this.__scheduleSwitchBackToMainProxy();
    }
}

function __scheduleSwitchBackToMainProxy(this: IungoWebPhoneTransport): void {
...
function __clearSwitchBackToMainProxyTimer(this: IungoWebPhoneTransport): void {
    if (this.switchBackToMainProxyTimer) {
...
async function reconnect(this: IungoWebPhoneTransport, forceReconnectToMain?: boolean): Promise<void> {
...
function getNextWsServer(this: IungoWebPhoneTransport, force = false): TransportServer {
...
function noAvailableServers(this: IungoWebPhoneTransport): boolean {
    return this.servers.every(({isError}) => isError);
}

function isSipErrorCode(this: IungoWebPhoneTransport, statusCode: number | undefined): boolean {
    if (!statusCode) {
        return false;
    }
...
async function onSipErrorCode(this: IungoWebPhoneTransport): Promise<void> {
