...
export const startQosStatsCollection = (session: IungoWebPhoneSession): void => {
...
const publishQosStats = async (session: IungoWebPhoneSession, qosStatsObj: QosStats, options: any = {}): Promise<void> => {
    options = options || {};

    const targetUrl = options.targetUrl || 'sip:iungo@dev2.iungo.cloud:5080';
    const event = options.event || 'vq-iungo';
    options.expires = 60;
    options.contentType = 'application/vq-iungo';
    options.extraHeaders = (options.extraHeaders || []).concat(session.userAgent.defaultHeaders);
    const cpuOS = session.__qosStats.cpuOS;
    const cpuIUNGO = session.__qosStats.cpuIUNGO;
    ...
    options.extraHeaders.push(
        `p-iungo-client-info:cpuIUNGO=${cpuIUNGO};cpuOS=${cpuOS};netType=${networkType};ram=${ram};effectiveType=${effectiveType}`
    );
