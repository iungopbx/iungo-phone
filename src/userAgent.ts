...
import {IungoWebPhoneTransport, createIungoWebPhoneTransport} from './transport';

import {SipInfo, IungoWebPhoneOptions} from './index';
...
import {
    ...
    IungoHeaders,
    IungoWebPhoneInvitation,
    IungoWebPhoneSession
} from './session';
import {patchUserAgentCore} from './userAgentCore';

/** Iungo Active call info */
...
/**
 * IungoWebPhoneUserAgent that makes SIP calls on behalf of the user
 */
export interface IungoWebPhoneUserAgent extends UserAgent {
    ...
    /**
     * @internal
     * Contains list of default headers needed to be sent to Iungo SIP server
     */
    ...
    /** If `true`, Qality of service of the call is generated and published to Iungo server once the call ends */
    enableQos?: boolean;
    ...
    /** sip info received by Iungo backend server when provisioning a device */
    sipInfo?: SipInfo;
    /** Transport class over which communication would take place */
    transport: IungoWebPhoneTransport;
    ...
    /**
     * @internal
     * Helper function to create Iungo message
     */
    createRcMessage?: (options: IungoHeaders) => string;
    ...
    invite?: (number: string, options: InviteOptions) => IungoWebPhoneSession;
     ...
     */
    onSession?: (session: IungoWebPhoneSession) => void;
    ...
    switchFrom?: (activeCall: ActiveCallInfo, options: InviteOptions) => IungoWebPhoneSession;
...
/** @ignore */
export function createIungoWebPhoneUserAgent(
    configuration: UserAgentOptions,
    sipInfo: SipInfo,
    options: IungoWebPhoneOptions,
    id: string
): IungoWebPhoneUserAgent {
    const extraConfiguration: UserAgentOptions = {
        delegate: {
            onConnect: (): Promise<void> => userAgent.register(),
            onInvite: (invitation: IungoWebPhoneInvitation): void => {
    ...
    const userAgent: IungoWebPhoneUserAgent = new UserAgent(extendedConfiguration) as IungoWebPhoneUserAgent;
    ...
    userAgent.defaultHeaders = [`P-iungo-endpoint-id: ${id}`, `Client-id: ${options.clientId}`];
    ...
    (userAgent as any)._transport = createIungoWebPhoneTransport(userAgent.transport, options);
...
function onTransportDisconnect(this: IungoWebPhoneUserAgent, error?: Error): void {
    // Patch it so that reconnection is managed by IungoWebPhoneTransport
...
function createRcMessage(this: IungoWebPhoneUserAgent, options: IungoHeaders): string {
...
function sendMessage(this: IungoWebPhoneUserAgent, to: string, messageData: string): Promise<IncomingResponse> {
    const extraHeaders = [`P-iungo-ws: ${this.contact}`];
...
async function register(this: IungoWebPhoneUserAgent): Promise<void> {
...
async function unregister(this: IungoWebPhoneUserAgent): Promise<void> {
    await this.registerer.unregister();
}

function invite(this: IungoWebPhoneUserAgent, number: string, options: InviteOptions = {}): IungoWebPhoneSession {
    ...
    inviterOptions.extraHeaders = [
        ...
        ...(options.homeCountryId ? [`P-iungo-country-id: ${options.homeCountryId}`] : [])
    ];
    ...
    const inviter: IungoWebPhoneSession = new Inviter(
        this,
...
function switchFrom(this: IungoWebPhoneUserAgent, activeCall: ActiveCallInfo, options: InviteOptions = {}): IungoWebPhoneSession {
